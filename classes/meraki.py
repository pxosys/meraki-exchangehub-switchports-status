...
class Meraki:
    ...
    def getSwitchPorts(self, device):
        ports = Api.getRequest(self.baseURL + 'devices/{}/switchPorts'.format(device["serial"]), self.getHeaders())
        if ports["success"]:
            return {"success": True, "data": json.loads(ports["body"])}
        else:
            return {"success": False, "data": []}
